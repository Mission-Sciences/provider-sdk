...
> Note: Model provider context — AWS Bedrock (Anthropic). Primary model: `us.anthropic.claude-4-sonnet-20250514-v1:0` (Sonnet 4). Fallback: `us.anthropic.claude-3-7-sonnet-20250219-v1:0` (Sonnet 3.7). ECS credentials via task role.

## Primary Objective

Transform integration requirements from planning/REQUIREMENTS.md (PRP-based) into MINIMAL tool specifications. Focus on the 2-3 essential tools needed for the agent to work. Avoid creating tools "just in case."
...
### 2. Agent Dependencies (Standard Pattern)

Follow established patterns from examples/agents/ for consistent dependency structure:
...
class AgentDependencies:
    """Standard dependencies following examples/ patterns"""
    # Core infrastructure (always present)
    redis_client: redis.Redis  # ElastiCache connection via get_redis_client()
    whisper_server: Any  # MCP server connection via create_whisper_server()
    agent_name: str  # Agent identification for sessions
    session_id: str  # Unique session ID for this agent run

    # Tool-specific dependencies (add as needed)
    search_api_key: Optional[str] = None  # Example: web search API
    database_client: Optional[Any] = None  # Example: database connection
```

### 3. Tool Implementation Standards
...
## Redis ElastiCache Tool Patterns

Tools using pottery utils for distributed operations (following examples/agents/ patterns):
...
async def queue_task(
    ...
    """Queue a task for distributed processing using pottery utils."""
    from redis_utils.pottery_utils import enqueue_item

    try:
        success = enqueue_item(
            redis_client=ctx.deps.redis_client,
            queue_name=f"{queue_name}:{ctx.deps.session_id}",
            item_data=task_data
        )
        return {"success": success, "queued": task_data}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def acquire_lock(
    ...
    """Acquire distributed lock for exclusive resource access."""
    from redis_utils.pottery_utils import acquire_pottery_lock

    lock_resource = f"{resource_name}:{ctx.deps.agent_name}"
    success, lock_info = acquire_pottery_lock(
        redis_client=ctx.deps.redis_client,
...
Tools for Whisper MCP server integration:
...
async def create_whisper(
    ...
    """Create a new whisper via MCP server."""
    try:
        # Use whisper_server from dependencies (connected via create_whisper_server())
        result = await ctx.deps.whisper_server.call_tool(
            "create_whisper",
            {
                "name": name,
                "headline": headline,
        ...
        return {"success": True, "whisper_id": result.get("id")}
    except Exception as e:
        return {"success": False, "error": str(e)}
```
