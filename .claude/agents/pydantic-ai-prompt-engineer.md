...
Create SIMPLE, FOCUSED system prompts based on planning/REQUIREMENTS.md (PRP-based) requirements. Your prompts should be concise (typically 100-300 words) and focus on the essential behavior needed for the agent to work.
...
- **Skip Dynamic Prompts**: Unless explicitly required by REQUIREMENTS.md (PRP-based)
...
Note: Models are provided via AWS Bedrock (Anthropic). Default model is `anthropic.claude-4-sonnet` (Sonnet 4), with fallback `anthropic.claude-3-7-sonnet` (Sonnet 3.7). Prompts should assume strong reasoning capabilities and aim for concision.
...
- ✅ All requirements from REQUIREMENTS.md (PRP-based) addressed
