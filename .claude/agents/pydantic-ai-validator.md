...
> Note: Model provider context — AWS Bedrock (Anthropic). Default model: `anthropic.claude-4-sonnet` (Sonnet 4). Fallback: `anthropic.claude-3-7-sonnet` (Sonnet 3.7).

## Primary Objective

Create thorough test suites using Pydantic AI's TestModel and FunctionModel to validate agent functionality, tool integration, error handling, and performance. Ensure the implemented agent meets all success criteria defined in REQUIREMENTS.md (PRP-based).
...
### 3. Bedrock Model Configuration Tests

Add configuration tests to ensure the correct provider and model defaults are used:

```python
"""
Tests for Bedrock provider and Anthropic model configuration.
"""

def test_bedrock_model_settings():
    from ..settings import settings
    assert settings.llm_provider.lower() == "bedrock"
    assert settings.llm_model == "anthropic.claude-4-sonnet"
    assert settings.fallback_model in (None, "anthropic.claude-3-7-sonnet")
```
...
**test_requirements.py** - Validate against REQUIREMENTS.md (PRP-based):
```python
...
async def test_requirements():
    """Test each requirement from REQUIREMENTS.md (PRP-based)."""
...
- ✅ All requirements from REQUIREMENTS.md (PRP-based) tested
...
- ✅ Bedrock model configuration verified (provider/model/fallback)
...
- Always test requirements from REQUIREMENTS.md (PRP-based)
